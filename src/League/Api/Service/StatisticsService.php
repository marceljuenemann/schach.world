        ...
        if ($pairing->team1->id == $team['team']->id) {
          foreach ($pairing->games->getValues() as $game) {
            if (is_object($game->player1)) {
              ...
              if (!in_array($game->player1->id, $active_players_ids)) {
                $active_players_ids[] = $game->player1->id;
                $team['players'][$game->player1->id] = $game->player1;
              }
            }
          }
        }
        if ($pairing->team2->id == $team['team']->id) {
          foreach ($pairing->games->getValues() as $game) {
            if (is_object($game->player2)) {
              ...
              if (!in_array($game->player2->id, $active_players_ids)) {
                $active_players_ids[] = $game->player2->id;
                $team['players'][$game->player2->id] = $game->player2;
              }
            }
          }
        }
 ...
 */
public
function active_teams_with_players($active_players, $division) {
  $team_repository = $this->doctrine->getRepository(Team::class);
  $teams_with_pairings = $this->teams_with_active_players($division);
 ...
 */
public
function active_players_with_games($active_players, $all_games) {
 ...
 */
public
function gameCountPlayer($active_players_with_games) {
  $players_games_count = [];
 ...
 */
public
function teams_dwz_calculation($active_teams_with_players, $active_players_with_games) {
 ...
 */
public
function active_teams_with_parings($division) {
 ...
 */
public
function team_game_score_data($active_teams_with_parings) {
  $teams_game_scores = [];
 ...
 */
public
function players_sorted_by_player_number($players) {
 ...
 */
public
function players_sorted_by_points_and_games($active_players_with_games) {
 ...
 */
public
function players_sorted_by_draws($active_players_with_games) {
  uasort($active_players_with_games, function ($a, $b) {
 ...
 */
public
function create_dwz_statistics_table($division) {
  $all_games = $this->all_games_division($division);
 ...
 */
public
function create_topscorer_table($division) {
  $all_games = $this->all_games_division($division);
  $active_players = $this->active_players_division($all_games);
  $active_players_with_games = $this->active_players_with_games($active_players, $all_games);
