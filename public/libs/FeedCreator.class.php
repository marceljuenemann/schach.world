	...
	function output() {
		...
		if (!$this->rawFieldContent) {
			...
			if ($this->truncSize and is_int($this->truncSize)) {
				$result = FeedCreator::iTrunc(isohtmlspecialchars($this->rawFieldContent),$this->truncSize);
			} else {
				$result = isohtmlspecialchars($this->rawFieldContent);
			}
		}
		return $result;
	}
...
class RSSCreator10 extends FeedCreator {
	...
	function createFeed() {     
		...
		$feed.= "        <title>".isohtmlspecialchars($this->title)."</title>\n";
		$feed.= "        <description>".isohtmlspecialchars($this->description)."</description>\n";
		$feed.= "        <link>".$this->link."</link>\n";
		if ($this->image!=null) {
			$feed.= "        <image rdf:resource=\"".$this->image->url."\" />\n";
		}
		$now = new FeedDate();
		$feed.= "       <dc:date>".isohtmlspecialchars($now->iso8601())."</dc:date>\n";
		$feed.= "        <items>\n";
		$feed.= "            <rdf:Seq>\n";
		for ($i=0;$i<count($this->items);$i++) {
			$feed.= "                <rdf:li rdf:resource=\"".isohtmlspecialchars($this->items[$i]->link)."\"/>\n";
		}
		$feed.= "            </rdf:Seq>\n";
		$feed.= "        </items>\n";
		$feed.= "    </channel>\n";
		...
		for ($i=0;$i<count($this->items);$i++) {
			$feed.= "    <item rdf:about=\"".isohtmlspecialchars($this->items[$i]->link)."\">\n";
			//$feed.= "        <dc:type>Posting</dc:type>\n";
			$feed.= "        <dc:format>text/html</dc:format>\n";
			if ($this->items[$i]->date!=null) {
				$itemDate = new FeedDate($this->items[$i]->date);
				$feed.= "        <dc:date>".isohtmlspecialchars($itemDate->iso8601())."</dc:date>\n";
			}
			if ($this->items[$i]->source!="") {
				$feed.= "        <dc:source>".isohtmlspecialchars($this->items[$i]->source)."</dc:source>\n";
			}
			if ($this->items[$i]->author!="") {
				$feed.= "        <dc:creator>".isohtmlspecialchars($this->items[$i]->author)."</dc:creator>\n";
			}
			$feed.= "        <title>".isohtmlspecialchars(strip_tags(strtr($this->items[$i]->title,"\n\r","  ")))."</title>\n";
			$feed.= "        <link>".isohtmlspecialchars($this->items[$i]->link)."</link>\n";
			$feed.= "        <description>".isohtmlspecialchars($this->items[$i]->description)."</description>\n";
...
class RSSCreator091 extends FeedCreator {
	...
	function createFeed() {
		...
		$feed.= "    <channel>\n";
		$feed.= "        <title>".FeedCreator::iTrunc(isohtmlspecialchars($this->title),100)."</title>\n";
		$this->descriptionTruncSize = 500;
		$feed.= "        <description>".$this->getDescription()."</description>\n";
		$feed.= "        <link>".$this->link."</link>\n";
		$now = new FeedDate();
		$feed.= "        <lastBuildDate>".isohtmlspecialchars($now->rfc822())."</lastBuildDate>\n";
		$feed.= "        <generator>".FEEDCREATOR_VERSION."</generator>\n";

		if ($this->image!=null) {
			$feed.= "        <image>\n";
			$feed.= "            <url>".$this->image->url."</url>\n"; 
			$feed.= "            <title>".FeedCreator::iTrunc(isohtmlspecialchars($this->image->title),100)."</title>\n"; 
			$feed.= "            <link>".$this->image->link."</link>\n";
			if ($this->image->width!="") {
		...
		if ($this->copyright!="") {
			$feed.= "        <copyright>".FeedCreator::iTrunc(isohtmlspecialchars($this->copyright),100)."</copyright>\n";
		}
		if ($this->editor!="") {
			$feed.= "        <managingEditor>".FeedCreator::iTrunc(isohtmlspecialchars($this->editor),100)."</managingEditor>\n";
		}
		if ($this->webmaster!="") {
			$feed.= "        <webMaster>".FeedCreator::iTrunc(isohtmlspecialchars($this->webmaster),100)."</webMaster>\n";
		}
		if ($this->pubDate!="") {
			$pubDate = new FeedDate($this->pubDate);
			$feed.= "        <pubDate>".isohtmlspecialchars($pubDate->rfc822())."</pubDate>\n";
		}
		if ($this->category!="") {
			$feed.= "        <category>".isohtmlspecialchars($this->category)."</category>\n";
		}
		if ($this->docs!="") {
			$feed.= "        <docs>".FeedCreator::iTrunc(isohtmlspecialchars($this->docs),500)."</docs>\n";
		}
		if ($this->ttl!="") {
			$feed.= "        <ttl>".isohtmlspecialchars($this->ttl)."</ttl>\n";
		}
		if ($this->rating!="") {
			$feed.= "        <rating>".FeedCreator::iTrunc(isohtmlspecialchars($this->rating),500)."</rating>\n";
		}
		if ($this->skipHours!="") {
			$feed.= "        <skipHours>".isohtmlspecialchars($this->skipHours)."</skipHours>\n";
		}
		if ($this->skipDays!="") {
			$feed.= "        <skipDays>".isohtmlspecialchars($this->skipDays)."</skipDays>\n";
		}
		$feed.= $this->_createAdditionalElements($this->additionalElements, "    ");

		for ($i=0;$i<count($this->items);$i++) {
			$feed.= "        <item>\n";
			$feed.= "            <title>".FeedCreator::iTrunc(isohtmlspecialchars(strip_tags($this->items[$i]->title)),100)."</title>\n";
			$feed.= "            <link>".isohtmlspecialchars($this->items[$i]->link)."</link>\n";
			$feed.= "            <description>".$this->items[$i]->getDescription()."</description>\n";
			
			if ($this->items[$i]->author!="") {
				$feed.= "            <author>".isohtmlspecialchars($this->items[$i]->author)."</author>\n";
			}
			/*
			// on hold
			if ($this->items[$i]->source!="") {
					$feed.= "            <source>".isohtmlspecialchars($this->items[$i]->source)."</source>\n";
			}
			*/
			if ($this->items[$i]->category!="") {
				$feed.= "            <category>".isohtmlspecialchars($this->items[$i]->category)."</category>\n";
			}
			if ($this->items[$i]->comments!="") {
				$feed.= "            <comments>".isohtmlspecialchars($this->items[$i]->comments)."</comments>\n";
			}
			if ($this->items[$i]->date!="") {
			$itemDate = new FeedDate($this->items[$i]->date);
				$feed.= "            <pubDate>".isohtmlspecialchars($itemDate->rfc822())."</pubDate>\n";
			}
			if ($this->items[$i]->guid!="") {
				$feed.= "            <guid>".isohtmlspecialchars($this->items[$i]->guid)."</guid>\n";
			}
			$feed.= $this->_createAdditionalElements($this->items[$i]->additionalElements, "        ");
			$feed.= "        </item>\n";
		}
...
class PIECreator01 extends FeedCreator {
	...
	function createFeed() {
		...
		for ($i=0;$i<count($this->items);$i++) {
			...
			$feed.= "        <link>".isohtmlspecialchars($this->items[$i]->link)."</link>\n";
			$itemDate = new FeedDate($this->items[$i]->date);
			$feed.= "        <created>".isohtmlspecialchars($itemDate->iso8601())."</created>\n";
			$feed.= "        <issued>".isohtmlspecialchars($itemDate->iso8601())."</issued>\n";
			$feed.= "        <modified>".isohtmlspecialchars($itemDate->iso8601())."</modified>\n";
			$feed.= "        <id>".isohtmlspecialchars($this->items[$i]->guid)."</id>\n";
			if ($this->items[$i]->author!="") {
				$feed.= "        <author>\n";
				$feed.= "            <name>".isohtmlspecialchars($this->items[$i]->author)."</name>\n";
...
class AtomCreator03 extends FeedCreator {
	...
	function createFeed() {
		...
		$feed.= ">\n"; 
		$feed.= "    <title>".isohtmlspecialchars($this->title)."</title>\n";
		$feed.= "    <tagline>".isohtmlspecialchars($this->description)."</tagline>\n";
		$feed.= "    <link rel=\"alternate\" type=\"text/html\" href=\"".isohtmlspecialchars($this->link)."\"/>\n";
		$feed.= "    <id>".isohtmlspecialchars($this->link)."</id>\n";
		$now = new FeedDate();
		$feed.= "    <modified>".isohtmlspecialchars($now->iso8601())."</modified>\n";
		if ($this->editor!="") {
			$feed.= "    <author>\n";
		...
		for ($i=0;$i<count($this->items);$i++) {
			$feed.= "    <entry>\n";
			$feed.= "        <title>".isohtmlspecialchars(strip_tags($this->items[$i]->title))."</title>\n";
			$feed.= "        <link rel=\"alternate\" type=\"text/html\" href=\"".isohtmlspecialchars($this->items[$i]->link)."\"/>\n";
			if ($this->items[$i]->date=="") {
			...
			$feed.= "        <issued>".isohtmlspecialchars($itemDate->iso8601())."</issued>\n";
			$feed.= "        <modified>".isohtmlspecialchars($itemDate->iso8601())."</modified>\n";
			$feed.= "        <id>".isohtmlspecialchars($this->items[$i]->link)."</id>\n";
			$feed.= $this->_createAdditionalElements($this->items[$i]->additionalElements, "        ");
			if ($this->items[$i]->author!="") {
				$feed.= "        <author>\n";
				$feed.= "            <name>".isohtmlspecialchars($this->items[$i]->author)."</name>\n";
			...
			if ($this->items[$i]->description!="") {
				$feed.= "        <summary>".isohtmlspecialchars($this->items[$i]->description)."</summary>\n";
			}
			$feed.= "    </entry>\n";
		}
		$feed.= "</feed>\n";
...
class OPMLCreator extends FeedCreator {
	...
	function createFeed() {     
		...
		for ($i=0;$i<count($this->items);$i++) {
			...
			$title = isohtmlspecialchars(strip_tags(strtr($this->items[$i]->title,"\n\r","  ")));
			$feed.= " title=\"".$title."\"";
			$feed.= " text=\"".$title."\"";
			//$feed.= " description=\"".isohtmlspecialchars($this->items[$i]->description)."\"";
			$feed.= " url=\"".isohtmlspecialchars($this->items[$i]->link)."\"";
			$feed.= "/>\n";
		}
		$feed.= "    </body>\n";
		$feed.= "</opml>\n";
	...
	function createFeed() {
		...
		if ($this->image!=null) {
			$imageStr = "<a href='".$this->image->link."'".$targetInsert.">".
							...
							FeedCreator::iTrunc(isohtmlspecialchars($this->image->title),100).
		...
		if ($this->title) {
			$feedArray[] = "<div class='".$this->stylePrefix."title'><a href='".$this->link."' ".$targetInsert." class='".$this->stylePrefix."title'>".
				FeedCreator::iTrunc(isohtmlspecialchars($this->title),100)."</a></div>";
		}
			...
			if ($this->items[$i]->title) {
				if ($this->items[$i]->link) {
					$feedArray[] = 
						...
						"item_title'".$targetInsert.">".FeedCreator::iTrunc(isohtmlspecialchars(strip_tags($this->items[$i]->title)),100).
						"</a></div>";
				} else {
					$feedArray[] = 
						"<div class='".$this->stylePrefix."item_title'>".
						FeedCreator::iTrunc(isohtmlspecialchars(strip_tags($this->items[$i]->title)),100).
						"</div>";
				}
			}
